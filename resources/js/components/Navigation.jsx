...
export default function Navigation() {
  ...
  const desktopLink = ({ isActive }) =>
    `font-mono text-xs tracking-wide transition-all px-0.5 pb-0.5 border-b ${
      isActive ? 'border-gray-900 text-gray-900 font-medium' : 'border-transparent text-gray-400 hover:text-gray-700 hover:border-gray-300'
    }`;

  const mobileLink = ({ isActive }) =>
    `flex items-center px-4 py-2.5 font-mono text-xs tracking-wide border-l-2 transition-all ${
      isActive ? 'border-gray-900 text-gray-900 bg-gray-50 font-medium' : 'border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-50'
    }`;

  const initials = (name = '') => name.split(' ').map((w) => w[0]).join('').toUpperCase().slice(0, 2) || '??';

  return (
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-50" style={{ fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14">
          <NavLink to="/dashboard" className="flex items-center gap-2.5 flex-shrink-0">
            ...
            <span className="hidden sm:block font-mono text-xs tracking-widest text-gray-400 uppercase">Inventory</span>
          </NavLink>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:block relative">
              ...
              >
                <div
                  className="w-5 h-5 rounded bg-gray-900 flex items-center justify-center font-mono text-xs font-medium text-white leading-none flex-shrink-0"
                  style={{ fontSize: '9px' }}
                >
                  {initials(user?.name)}
                </div>
                <span className="font-mono text-xs text-gray-600 max-w-[96px] truncate">{user?.name || 'Account'}</span>
                <svg
                  width="10"
                  height="6"
                  viewBox="0 0 10 6"
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-1.5 w-40 bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden z-50">
                  ...
                  <button onClick={handleLogout} className="w-full text-left px-3 py-2 font-mono text-xs text-red-600 hover:bg-red-50 transition-colors">
                    Sign out
                  </button>
                </div>
              )}
      ...
      {isOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white">
          <div className="py-2">
            {NAV_ITEMS.map(({ to, label }) => (
              <NavLink key={to} to={to} className={mobileLink} onClick={() => setIsOpen(false)}>
                {label}
              </NavLink>
            ))}
          </div>
          ...
          <div className="border-t border-gray-100 px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-xs text-gray-600 truncate max-w-[160px]">{user?.name || 'Account'}</span>
            </div>
            <button onClick={handleLogout} className="font-mono text-xs text-red-600 hover:text-red-800 transition-colors">
              Sign out
            </button>
          </div>
        </div>
