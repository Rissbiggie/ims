...
export default function CategoriesPage() {
  ...
  const CHIP_COLORS = ['bg-blue-50 text-blue-700', 'bg-amber-50 text-amber-700', 'bg-green-50 text-green-700', 'bg-purple-50 text-purple-700', 'bg-red-50 text-red-700', 'bg-teal-50 text-teal-700'];
  const chipColor = (name = '') => CHIP_COLORS[name.charCodeAt(0) % CHIP_COLORS.length];
  ...
  if (loading)
    return (
      <div className="flex items-center justify-center min-h-screen">
        <span className="font-mono text-xs tracking-widest text-gray-300 uppercase animate-pulse">Loading categories...</span>
      </div>
    );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" style={{ fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        ...
        <div className="bg-gray-50 rounded-md px-4 py-3 hidden md:block">
          ...
          <p className="font-mono text-xl font-semibold text-gray-900">
            {categories.length ? Math.round(categories.reduce((s, c) => s + (c.name?.length || 0), 0) / categories.length) : 0}{' '}
            <span className="text-sm font-normal text-gray-400">chars</span>
          </p>
        </div>
      </div>
      ...
      {showForm && (
        <div className="bg-white border border-gray-200 rounded-md p-6 mb-8">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="font-mono text-xs font-semibold tracking-widest text-gray-500 uppercase">{editId ? 'Edit Category' : 'Add Category'}</h2>
          </div>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2 border-t border-gray-100">
              <button type="submit" className="font-mono text-xs font-medium px-5 py-2.5 bg-gray-900 text-white rounded hover:bg-gray-700 transition-all">
      ...
      {categories.length === 0 ? (
        <div className="text-center py-16 font-mono text-xs text-gray-300 uppercase tracking-widest">No categories on record.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {categories.map((category) => (
            <div key={category.id} className="bg-white border border-gray-200 rounded-md p-5 flex flex-col gap-3 hover:border-gray-300 transition-colors">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <h3 className="font-medium text-gray-900 text-sm leading-snug truncate">{category.name}</h3>
                </div>
                <span className="font-mono text-xs text-gray-300 flex-shrink-0">#{String(category.id).padStart(3, '0')}</span>
              </div>
              ...
              <div className="flex gap-2 pt-2 border-t border-gray-100">
                <button onClick={() => handleEdit(category)} className="font-mono text-xs px-2.5 py-1 rounded border border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 transition-all">
                  Edit
                </button>
                <button onClick={() => handleDelete(category.id)} className="font-mono text-xs px-2.5 py-1 rounded border border-red-200 bg-red-50 text-red-700 hover:bg-red-100 transition-all">
                  Delete
                </button>
              </div>
            </div>
