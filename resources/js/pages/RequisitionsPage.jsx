...
export default function RequisitionsPage() {
  ...
  const fetchData = async () => {
    try {
      const [reqRes, prodRes] = await Promise.all([requisitionsApi.getAll(), productsApi.list()]);
  ...
  const handleSubmit = async (e, shouldSubmitImmediately = false) => {
    ...
    try {
      await requisitionsApi.create({
        ...formData,
        submit: shouldSubmitImmediately,
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 antialiased min-h-screen bg-[#fafafa]" style={{ fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500&family=IBM+Plex+Sans:wght@300;400;600&display=swap');`}</style>
        ...
        <div className="bg-white border border-gray-200 rounded shadow-sm p-8 mb-12 animate-in fade-in slide-in-from-top-4">
          <form onSubmit={(e) => handleSubmit(e, false)} className="space-y-8">
            ...
            <div className="pt-6 border-t border-gray-50">
              <div className="flex justify-between items-center mb-6">
                ...
                <button
                  ...
                  className="text-[10px] font-mono border border-gray-200 px-4 py-2 rounded hover:bg-gray-50 uppercase tracking-widest transition-all"
                >
                  + Add_Entry
                </button>
              </div>
                ...
                {items.map((item, i) => (
                  <div key={i} className="grid grid-cols-[3fr_1fr_auto] gap-4 items-center animate-in fade-in">
                    ...
                    <input
                      ...
                      className="text-sm px-4 py-3 bg-gray-50 border border-gray-100 rounded outline-none"
                    />
                    <button type="button" onClick={() => handleRemoveItem(i)} className="text-gray-300 hover:text-red-500 p-2 transition-colors">
                      ✕
                    </button>
                  </div>
                ))}
            ...
            <div className="flex gap-4 pt-8">
              <button
                type="submit"
                className="font-mono text-[10px] font-bold px-8 py-4 border border-black hover:bg-gray-50 uppercase tracking-widest transition-all"
              >
                SAVE_AS_DRAFT
              </button>
              <button
                ...
                className="font-mono text-[10px] font-bold px-8 py-4 bg-black text-white hover:bg-gray-800 uppercase tracking-widest transition-all shadow-lg"
              >
                SUBMIT_FOR_APPROVAL
              </button>
            </div>
          ...
          <tbody>
            {requisitions.map((req) => (
              <tr key={req.id} className="hover:bg-gray-50 transition-colors border-b border-gray-50">
                ...
                <td className="px-4 py-5">
                  <span
                    className={`font-mono text-[9px] font-bold px-3 py-1.5 rounded-full border uppercase tracking-[0.15em] ${
                ...
                <td className="px-4 py-5">
                  <div className="flex gap-2">
                    {req.status === 'draft' && (
                      <button onClick={() => handleAction(req.id, 'submit')} className="text-[10px] font-mono font-bold text-blue-600 hover:underline tracking-widest">
                        SUBMIT
                      </button>
                    )}
                    {req.status === 'pending' && (
                      <>
                        <button onClick={() => handleAction(req.id, 'approve')} className="text-[10px] font-mono font-bold text-emerald-600 hover:underline tracking-widest">
                          APPROVE
                        </button>
                        <button onClick={() => handleAction(req.id, 'reject')} className="text-[10px] font-mono font-bold text-red-600 hover:underline tracking-widest">
                          REJECT
                        </button>
                      </>
                    )}
                    {req.status === 'approved' && (
                      <button onClick={() => handleAction(req.id, 'issue')} className="text-[10px] font-mono font-bold text-black hover:underline tracking-widest underline-offset-4">
                        EXECUTE_ISSUE
                      </button>
                    )}
                  </div>
