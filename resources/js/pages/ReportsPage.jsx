...
export default function ReportsPage() {
  ...
  const KSH = (val) => new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(val);
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 antialiased min-h-screen bg-[#fafafa]" style={{ fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div className="flex justify-between items-end mb-12 no-print">
        ...
        <div className="flex gap-3">
          <button
            onClick={handlePrint}
            disabled={loading}
            className="font-mono text-[10px] font-bold px-6 py-3 border border-gray-200 rounded hover:bg-white shadow-sm transition-all disabled:opacity-50"
          >
          ...
          <button
            onClick={loadReport}
            disabled={loading}
            className="font-mono text-[10px] font-bold px-6 py-3 bg-black text-white rounded hover:bg-gray-800 shadow-lg transition-all disabled:opacity-50"
          >
            EXECUTE_QUERY
          </button>
        </div>
        ...
        <div className="flex flex-wrap">
          {TABS.map(({ id, label }) => (
            <button
              ...
              className={`font-mono text-[10px] tracking-widest px-6 py-4 border-b-2 transition-all ${
                activeTab === id ? 'border-black text-black font-bold' : 'border-transparent text-gray-400 hover:text-gray-600'
              }`}
            >
              {label.toUpperCase()}
            </button>
        ...
        {activeTab === 'movement' && (
          <div className="flex items-center gap-3 bg-white p-2 px-4 rounded-full border border-gray-200 shadow-sm mb-4 md:mb-0">
            <input
              ...
              className="bg-transparent font-mono text-[10px] focus:outline-none"
            />
      ...
      {error && (
        <div className="mb-8 p-4 bg-red-50 border border-red-100 text-red-600 font-mono text-[10px] rounded flex justify-between">
          <span>{error}</span>
          <button onClick={() => setError('')} className="underline font-bold italic">
            DISMISS
          </button>
        </div>
      )}
                    ...
                    {data.inventory?.length > 0 ? (
                      data.inventory.map((item) => (
                        <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <Td>
                            <span className="text-[10px] font-mono bg-gray-100 px-2 py-1 rounded">{item.category}</span>
                          </Td>
                          <Td mono right>
                            {item.quantity_on_hand}
                          </Td>
                          <Td mono right className="font-bold">
                      ...
                      <tr>
                        <Td colSpan="4" className="text-center py-20 text-gray-300 italic uppercase">
                          No_Inventory_Data
                        </Td>
                      </tr>
                    )}
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody>
                    {data.movement.transactions?.length > 0 ? (
                      data.movement.transactions.map((m) => (
                        <tr key={m.id} className="hover:bg-gray-50">
                          ...
                          <Td
                            mono
                            right
                            className={m.quantity_change > 0 ? 'text-green-600' : 'text-red-600'}
                          >
                      ...
                      <tr>
                        <Td colSpan="3" className="text-center py-20 text-gray-300 italic uppercase">
                          No_Movement_Logged
                        </Td>
                      </tr>
                    )}
                  ...
                  {data.lowStock?.length > 0 ? (
                    data.lowStock.map((item) => (
                      <tr key={item.id} className="hover:bg-red-50/30">
                        ...
                        <Td mono className="text-red-500">
                          {item.quantity_on_hand}
                        </Td>
                    ...
                    <tr>
                      <Td colSpan="3" className="py-20 text-center text-gray-300 font-mono italic uppercase">
                        All_Stock_Levels_Nominal
                      </Td>
                    </tr>
                  )}
                </tbody>
                  ...
                  {data.trend?.length > 0 ? (
                    data.trend.map((t, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <Td mono right className="text-green-600">
                          +{t.total_in}
                        </Td>
                        <Td mono right className="text-red-600">
                          -{t.total_out}
                        </Td>
                      </tr>
                    ))
                  ...
                  {data.topProducts?.length > 0 ? (
                    data.topProducts.map((p, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <Td mono right className="font-bold text-black bg-gray-50">
                    ...
                    <tr>
                      <Td colSpan="2" className="py-20 text-center text-gray-300 font-mono italic uppercase">
                        No_Frequent_Movers_Found
                      </Td>
                    </tr>
                  )}
                </tbody>
